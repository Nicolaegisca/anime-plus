document.addEventListener("DOMContentLoaded", async function () {
    ...
    function render() {
        ...
        var filtered = !genre
            ? animeList
            : animeList.filter(function (item) {
                return item.genre && item.genre.toLowerCase().indexOf(genre.toLowerCase()) !== -1;
            });

        if (filtered.length === 0) {
            if (emptyEl) {
                emptyEl.textContent = genre ? "No titles in this genre." : "Choose a genre above.";
        ...
        document.querySelectorAll(".genre-link").forEach(function (a) {
            var h = a.getAttribute("href") || "";
            var linkGenre = h.indexOf("genre=") !== -1 ? (h.split("genre=")[1].split("&")[0] || "").trim() : "";
            a.classList.toggle("genre-link-active", linkGenre === genre);
        });
    }
